...
@router.post("/upload_FOentry_csv")
async def upload_FOentry_csv(file: UploadFile = File(...)):
    ...
    for row in reader:
        ...
        if doc_id:
            db.collection('eq_FOentry_capture').document(doc_id).set(data)
        else:
            db.collection('eq_FOentry_capture').add(data)
        results.append(data)
