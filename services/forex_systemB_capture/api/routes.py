...
async def add_forex(forex: Forex):
    try:
        ...
        if forex_repository.get_forex(forex.TradeID):
            raise HTTPException(
                status_code=400,
                detail=f"TradeID '{forex.TradeID}' already exists")

        # Normalize and prepare forex data
        forex_data = {
            'Trade ID': forex.TradeID,
            'Instrument': forex.CurrencyPair,  # Fixed: use CurrencyPair instead of Instrument
            'Currency Pair': forex.CurrencyPair,
            'Quantity': forex.NotionalAmount,
            'Price': forex.FXRate,
            'Buy/Sell': forex.BuySell,
            'Settlement Date': forex.SettlementDate,
            'Counterparty': forex.Counterparty,
            'Product Type': forex.ProductType,
        }

        # Save to Firestore
        db = get_firestore_client()
        db.collection('fx_systemB_capture').document(
            forex.TradeID).set(forex_data)

        return {
            "message": "Successfully uploaded Forex data",
            "data": forex_data
        }
...
@router.post("/upload_systemB_csv")
async def upload_systemB_csv(file: UploadFile = File(...)):
    try:
        db = get_firestore_client()
        ...
        for row in reader:
            ...
            data = {
                ...
                'FX Rate': get_val('FX Rate', 'FXRate', 'FxRate', 'Rate'),
                'Notional Amount': get_val('Notional Amount', 'NotionalAmount', 'NotionalA'),
            ...
            if doc_id:
                db.collection('fx_systemB_capture').document(doc_id).set(data)
            else:
                db.collection('fx_systemB_capture').add(data)
            results.append(data)
